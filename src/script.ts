...
const getGenreFilms = (arr: TFilms[]): string[] =>
    arr.reduce((acc, genre) => {
        acc.indexOf(genre) === -1 && acc.push(genre);
        return acc;
    } , []);
...
const getFilmsRating = (arr: TFilms[]): number[] =>
    arr.map(users => users.imdbRating).sort((a, b) => b > a ? 1 : -1);

// 4. Создать новый массив, где объекты фильмов будут состоять из следующих
// полей:
// id, title, released, plot
const newArray = (arr: TFilms[]): {id: number, title: string, released: string, plot: string}[] =>
    arr.map(u => ({id: u.id, title: u.title, released: u.released, plot: u.plot}));
...
// const getArrayFilmsAndYears = (arr: TFilms[]): {title: string, year: number} =>
//     arr.reduce( u => ({
